...
> **Board decision (2026-06-08 - #Beacon-5398)** Halvorsen: escalation-pressure reach amplifier, final. A threat's escalation pressure now also reflects how far its chain propagates: the row's `chain_reach_score` (the strongest-path reach settled in #Beacon-5120) divided by 7 and FLOORED is ADDED to the pressure, so `escalation_pressure = chain_risk_score + ceil(carry_in / 3) + (chain_reach_score // 7)`. The carry-in credit rounding (ceil, #Beacon-5122), the gap decay (floor) and the carry-out chain-size debit (ceil, #Beacon-5394) are all unchanged; this only appends the reach term to the pressure that drives the escalation-critical flag and the ledger checksum. Because the amplifier reads the reach score, an error in the chain-reach propagation now also corrupts the escalation flags, the critical ids and the ledger checksum. ROUNDING: chain_reach_score // 7 = FLOOR.
...
> **Board decision (2026-06-08 - #Beacon-5396)** Halvorsen: override scope precedence, final. Re-examining suppression after the ledger review, the board tightens override matching on one point only: a threat candidate does NOT inherit the `all`-scoped windows when a window is recorded for its OWN (host_group, severity) scope. Concretely — if any suppression window exists for the candidate's own (host_group, severity), the candidate is matched ONLY against those own-severity windows and the `all`-scoped windows are not consulted for it; only candidates whose own (host_group, severity) has no window at all fall back to the `all`-scoped windows. The half-open comparison `start_ms <= seen_ms < end_ms` and everything else in #Beacon-5111 are unchanged. So a critical threat on a host_group that has its own critical window is judged solely by that critical window, even where an `all` window would otherwise have covered it. This supersedes the {all, candidate.severity} scope-set of #Beacon-5111.
